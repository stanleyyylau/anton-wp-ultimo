/* global _wuTemplateSettings, confirm */
/*jshint -W093 */
window.wp = window.wp || {};
...
themes.data = _wuTemplateSettings;
...
themes.view.Appearance = wp.Backbone.View.extend({
    ...
    events: {
        'click .filter-links li > a'  : 'search_category',
        'change div.wp-filter select' : 'search_select_category',
    },

    search_select_category: function (event) {

        event.preventDefault();

        var options = {};

        $('.filter-links li > a', this.el).removeClass('current');

        event.target.value = $(event.target).val();

        $("[data-category='"+ event.target.value  +"']").addClass('current');

        this.collection.doSearch( event.target.value );

        $( '.wp-filter-search' ).val( event.target.value );
    ...
    search_category: function( event ) {
        ...
        event.target.value = $(event.target).data('category');
    ...
    render: function() {
        ...
        // Render and append
        this.view.render();
        this.$el.find('.theme-browser').empty().append( this.view.el ).addClass( 'rendered' );
    },

    // Defines search element container
    searchContainer: $( '.wrap h1:first' ),
...
themes.Collection = Backbone.Collection.extend({
    ...
    search: function( term ) {
        ...
        results = this.filter( function( data ) {
            ...
            if (term === 'active') {
                return data.attributes.active === true;
            }

            if (term === 'inactive') {
                return data.attributes.active === false;
            }
...
themes.view.Theme = wp.Backbone.View.extend({
    ...
    render: function() {
        ...
        this.$el.addClass( 'col-' + _wuTemplateSettings.settings.cols );
...
//         this.$el.attr('data-slug', data.id);
...
var getRoutes = function() {
    ...
    /**
     *
        'plugins.php?plugin=:slug': 'theme',
        'plugins.php?search=:query': 'search',
        'plugins.php?s=:query': 'search',
        'plugins.php': 'themes',
        '': 'themes'
     */
    ...
    // routes[ wu_extensions.type + '.php?' + wu_extensions.type.replace('s', '') + '=:slug' ] = 'theme';
    // routes[ wu_extensions.type + '.php?search=:query' ] = 'search';
    // routes[ wu_extensions.type + '.php?s=:query' ] = 'search';
    // routes[ wu_extensions.type + '.php' ] = 'themes';
    // routes[ '' ] = 'themes';
...
//     routes: getRoutes(),
    ...
    baseUrl: function( url ) {
        return wu_extensions.type + '.php' + url;
    },

//     themePath: '?' + wu_extensions.type.replace('s', '') + '=',
//     searchPath: '?search=',

//     search: function( query ) {
//         $( '.wp-filter-search' ).val( query );

//         $('.filter-links li > a').removeClass('current');

//         var target = $('[data-category="'+ query +'"]');
        
//         if (target.length) {
//             target.addClass('current');
//         } else {
//             $('[data-category=""]').addClass('current');
//         }
...
$( document ).ready(function() {
    ...
    if (_wuTemplateSettings.selected_category !== false) {

        $('[data-category="' + _wuTemplateSettings.selected_category +'"]').click();

    } // end if;

    $( '.broken-themes .delete-theme' ).on( 'click', function() {
        return confirm( _wuTemplateSettings.settings.confirmDelete );
    });
});

$(document).ready(function () {

    if (_wuTemplateSettings.is_shortcode) {

        $('.theme-actions button').on('click', function(e) {
            e.preventDefault();

            var template_id = $(this).val();

            window.location.href = _wuTemplateSettings.register_url + "?template_id=" + template_id;
        });
