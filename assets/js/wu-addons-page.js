...
themes.Collection = Backbone.Collection.extend({
    ...
    search: function( term ) {
        ...
        results = this.filter( function( data ) {
            ...
            if (term === 'installed') {
                return data.attributes.installed === true;
            }
    ...
    paginate: function( instance ) {
        ...
        collection = _( collection.rest( 100 * instance ) );
        collection = _( collection.first( 100 ) );
...
var getRoutes = function() {
    ...
    var routes = {
        'network/admin.php?page=wp-ultimo-addons&addon=:slug': 'theme',
        'network/admin.php?page=wp-ultimo-addons&search=:query': 'search',
        'network/admin.php?page=wp-ultimo-addons&s=:query': 'search',
        'network/admin.php?page=wp-ultimo-addons': 'themes',
        '': 'themes'
    };
...
themes.Router = Backbone.Router.extend({
    ...
    baseUrl: function( url ) {
        return 'network/admin.php?page=wp-ultimo-addons' + url;
    },
...
(function($) {
  $(document).ready(function() {
    ...
    $('body').addClass('themes-php');

    $(document).on('click', '.gt-install', function(e) {
      ...
      $this.parents('.theme').addClass('focus');

      $this.html(_wpThemeSettings.l10n.installing)
        .addClass('theme-install')
        .addClass('updating-message');
      ...
      $.ajax({
        ...
        method: 'post',
        data: {
          action: 'wu_install_addon',
          nonce: _wpThemeSettings.nonce,
          download_url: $this.data('install-url')
        },
        // dataType: 'html',
        success: function(html) {
            ...
            var success      = response.find('.button-primary').length,
                text         = response.find('.button-primary').text(),
                activate_url = response.find('.button-primary').attr('href');


          if (success) {

            $this.parents('.theme').addClass('active');

            $this.html(text)
                .removeAttr('disabled')
                .removeClass('gt-install')
                .removeClass('theme-install')
                .removeClass('updating-message')
                .attr('href', activate_url); //.removeAttr('disabled');

          } else {

            $this.html(_wpThemeSettings.l10n.error)
                .removeAttr('disabled')
                .removeClass('theme-install')
                .removeClass('updating-message');
