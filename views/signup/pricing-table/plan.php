...
foreach(array(1, 3, 12) as $type) {
  ...
  $price = $plan->free ? __('Free!', 'wp-ultimo') : str_replace(get_wu_currency_symbol(), '', wu_format_currency( ( ( (float) $plan->{"price_".$type}) / $type )));
...
<div id="plan-<?php echo $plan->id; ?>" data-plan="<?php echo $plan->id; ?>" <?php echo $plan_attrs; ?> class="lift wu-plan plan-tier <?php echo $plan->top_deal ? 'callout' : ''; ?> wu-col-sm-<?php echo $columns; ?> wu-col-xs-12">
  ...
  <h4 class="wp-ui-primary"><?php echo $plan->title; ?></h4>
  ...
  <?php else : ?>
    ...
    <h5>
      <?php $symbol_left = in_array(WU_Settings::get_setting('currency_position', '%s%v'), array('%s%v', '%s %v')); ?>
      <?php if ($symbol_left) : ?><sup class="superscript"><?php echo get_wu_currency_symbol(); ?></sup><?php endif; ?>
      <span class="plan-price"><?php echo str_replace(get_wu_currency_symbol(), '', wu_format_currency($plan->price_1)); ?></span>
      <sub> <?php echo (! $symbol_left ? get_wu_currency_symbol() : '') . ' ' . __('/mo', 'wp-ultimo'); ?></sub>
    </h5>

  <?php endif; ?>
  <!-- end Price -->

  <p class="early-adopter-price"><?php echo $plan->description; ?>&nbsp;</p><br>
  ...
  <ul>
    ...
    foreach ($prices_total as $freq => $string) {
      ...
      $text = sprintf(__('%s, billed %s', 'wp-ultimo'), wu_format_currency($plan->{"price_$freq"}), $string);
