...
wp_localize_script( 'wu-addons-page', '_wpThemeSettings', array(
    ...
    'l10n' => array(
        'installing'        => __( 'Installing Feature Plugin', 'wp-ultimo'),
        'error'             => __( 'An error occured', 'wp-ultimo'),
        'addNew'            => __( 'Add New Plugin', 'wp-ultimo'),
        'search'            => __( 'Search available feature plugin', 'wp-ultimo'),
        'searchPlaceholder' => __( 'Search available feature plugin...', 'wp-ultimo'), // placeholder (no ellipsis)
        'themesFound'       => __( 'Number of Plugins found: %d', 'wp-ultimo'),
        'noThemesFound'     => __( 'No feature plugin found. Try a different search.', 'wp-ultimo'),
    ),
) );
...
?>
  <div class="wrap" id="wu-addon">
    ...
    <h1><?php esc_html_e( 'Feature Plugins', 'wp-ultimo' ); ?>
        <span class="title-count theme-count"><?php echo count( $addons ); ?></span>
    </h1>

    <p class="description">
      <?php _e('Feature Plugins are temporary plugins holding features that are being considered to be incorporated into WP Ultimo\'s core. <br>By having them separated into a single entity at first, we can make sure it does not break backwards compatibility and that it is easier to test and isolate failures.', 'wp-ultimo'); ?>
    </p>
    ...
    <div class="wp-filter">
      <ul class="filter-links">
        ...
        <li>
            <a href="#" class="current" data-category=""><?php _e('All Feature Plugins', 'wp-ultimo'); ?></a>
        </li>
      ...
      <?php
        ...
        */
        ...
        <div class="theme<?php if ( $addon['active'] ) echo ' active'; ?>" tabindex="0" aria-describedby="<?php echo $aria_action . ' ' . $aria_name; ?>">
          ...
          <span class="more-details" id="<?php echo $aria_action; ?>"><?php _e('Feature Plugin Details', 'wp-ultimo'); ?></span>
              ...
              <?php if ( $addon['sale'] ) { ?>
                ...
                <?php if ( $addon['golden_ticket'] ) { ?>
                  ...
                  <a class="button button-primary" href="<?php echo $addon['actions']['buy']; ?>">
                    <?php _e( 'Buy This Feature Plugin', 'wp-ultimo' ); ?>
                  </a>
...
?>
    <script id="tmpl-theme" type="text/template">
      ...
      <span class="more-details" id="{{ data.id }}-action"><?php _e('Feature Plugin Details', 'wp-ultimo'); ?></span>
