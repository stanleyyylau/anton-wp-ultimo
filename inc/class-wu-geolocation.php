 ...
 *
 * This class if a fork from WC_Geolocate, by WooCommerce.
 * This product includes GeoLite data created by MaxMind, available from http://www.maxmind.com.
 *
 * @author      Arindo Duque
 * @category    Admin
 * @package     WP_Ultimo/Classes
 * @version     0.0.1
 */
...
/**
 * WU_Geolocation Class.
 */
class WU_Geolocation {
	...
	public static function init() {
		...
		if ( 'geolocation' === get_option( 'wu_default_customer_address' ) || apply_filters( 'wu_geolocation_update_database_periodically', false ) ) {
	...
	public static function get_local_database_path( $version = 'v4' ) {
		...
		return apply_filters( 'wu_geolocation_local_database_path', $upload_dir['basedir'] . '/GeoIP' . $version . '.dat', $version );
	}
    ...
    $logger = new WU_Logger;

		if (!is_callable('gzopen')) {
			$logger->add('notice', 'Server does not support gzopen', array('source' => 'geolocation'));
			return;
		}
	...
	private static function geolocate_via_db($ip_address) {
		if (!class_exists('WU_Geo_IP', false)) {
			include_once WP_Ultimo()->path('inc/class-wc-geo-ip.php');
		}
