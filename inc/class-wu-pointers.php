...
class WU_Help_Pointers {
                ...
                $pointers[$ptr['id']] = array(
                    ...
                    'options' => array(
                        'content' => sprintf( '<h3> %s </h3> <p> %s </p>',
                            __( $ptr['title'] , 'wp-ultimo' ),
                            __( $ptr['content'], 'wp-ultimo' )
                        ),
    ...
    public function add_scripts() {
        ...
        jQuery(document).ready( function($) {
            ...
            function wp_help_nexted_pointers(pointers) {

                pointer = WPHelpPointer.pointers[current_pointer];

                var last_close = function() {
                    $.each(pointers, function(i, p) {
                        //console.log(i, p);
                        $.post(ajaxurl, {
                            pointer: p.pointer_id,
                ...
                var close = function() {
                    current_pointer++;
                    wp_help_nexted_pointers(pointers);
                };

                options = current_pointer + 1 === pointers.length ? $.extend( pointer.options, { close: last_close }) : $.extend( pointer.options, { close: close });

                $(pointer.target).pointer(options).pointer('open');

            }

            wp_help_nexted_pointers(WPHelpPointer.pointers);
