...
var EnvatoWizard = (function($){
  ...
  function PluginManager(){
        ...
        if(typeof response.url != 'undefined'){
          ...
          if(response.hash == current_item_hash){
            ...
            jQuery.post(response.url, response, function(response2) {
              process_current();
              $current_node.find('span').text(response.message + wu_setup_params.verify_text);
            }).fail(ajax_callback);
          }
    ...
    function process_current(){
      if(current_item){
        ...
        jQuery.post(wu_setup_params.ajaxurl, {
          action: 'wu_setup_plugins',
          wpnonce: wu_setup_params.wpnonce,
          slug: current_item
    ...
    function find_next(){
      ...
      var $li = $('.wpultimo-wizard-plugins li');
      $li.each(function(){
    ...
    return {
      init: function(btn){
        $('.wpultimo-wizard-plugins').addClass('installing');
        complete = function(){
          loading_content();
  ...
  function ContentManager(){
    ...
    return {
      init: function(btn){
        $('.wpultimo-setup-pages').addClass('installing');
        $('.wpultimo-setup-pages').find('input').prop("disabled", true);
        complete = function(){
          loading_content();
          window.location.href=btn.href;
        };
