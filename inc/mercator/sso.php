...
} // fix for wp-activation.php key cookie;
 ...
 */
function bootstrap() {
	add_action( 'wp_footer',          __NAMESPACE__ . '\\head_js', -100 );
	add_action( 'login_footer',          __NAMESPACE__ . '\\head_js', -100 );
...
function initialize_cookie_domain() {
	if ( empty( $GLOBALS['mercator_current_mapping'] ) ) {
		...
		if (substr($_SERVER['HTTP_HOST'], 0, 4) == 'www.') {
    		$c_domain = substr($_SERVER['HTTP_HOST'], 4);
		} else {
				$c_domain = $_SERVER['HTTP_HOST'];
		}

		define( 'COOKIE_DOMAIN', '.' . $c_domain);
...
function head_js() {
	...
	if (isset($_GET['reauth']) && $_GET['reauth']) {
		return;
	}
