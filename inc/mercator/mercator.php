...
function startup() {
	...
	// Too early to call `get_bloginfo('version')`;
	...
	add_action( $delete_blog_action, __NAMESPACE__ . '\\clear_mappings_on_delete' );
...
function mangle_url( $url, $path, $orig_scheme = 'http', $site_id = 0 ) {
	...
	$current_mapping = isset($GLOBALS['mercator_current_mapping']) ? $GLOBALS['mercator_current_mapping'] : false;
